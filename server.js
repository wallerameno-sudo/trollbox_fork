...
const HOME_SALT = process.env.HOME_SALT || 'server-pepper-change-me';
...
io.on('connection', (socket) => {
  ...
  socket.isSuperuser = authorizedHomes.has(homeHash);
    ...
    if (existingIndex !== -1) {
      ...
      if (existingUser.nick !== socket.nick) {
        io.emit('user change nick', [
          existingUser,
          { nick: socket.nick, color: socket.color, style: socket.style, home: socket.home }
        ]);
      }
  ...
  socket.on('message', (msg) => {
    ...
    if (socket.isSuperuser) {
      if (msg.startsWith('/ban ')) {
        const targetNick = msg.slice(5).trim();
        const targetUser = users.find(u => u.nick === targetNick);
        if (targetUser) {
          banHome(targetUser.home, 'banned by moderator');
        } else {
          sys.sendLogToClient(socket, `User with nick "${sanitizeHTML(targetNick)}" not found.`, true);
        }
        return;
        ...
        try {
          ...
          sys.sendLogToClient(socket, `<span style='color:red'>ERROR!</span>
            ${e.stack}`, false);
        }
        return;
      }
  ...
  socket.on('disconnect', () => {
    console.log('disconnect', socket.connId, socket.home);
